            ...
            if (node.superClass) {
              ...
              if (identifier) {
                ...
                if (getModuleInfo(moduleId, parser)) {
                  ...
                  lines[lines.length - 2] = ' * @extends ' + `${moduleId.replace(slashRegEx, '/')}${exportName ? delimiter + exportName : ''}`;
                }
              } else {
        ...
        node.comments.forEach(comment => {
          ...
          while ((importMatch = importRegEx.exec(comment.value))) {
            ...
            if (importMatch[1].charAt(0) !== '.') {
              ...
              replacement = `${importMatch[1]}${importMatch[3] === 'default' ? '' : '~' + importMatch[3]}`;
            } else {
              ...
              if (getModuleInfo(moduleId, parser)) {
                const exportName = !importMatch[3] ? '' : importMatch[3] === 'default' ? getDefaultExportName(moduleId, parser) : importMatch[3];
                const delimiter = !importMatch[3] ? '' : importMatch[3] === 'default' ? '~' : getDelimiter(moduleId, exportName, parser);
                replacement = `${moduleId.replace(slashRegEx, '/')}${exportName ? delimiter + exportName : ''}`;
              }
            }
            if (replacement) {
          ...
          Object.keys(identifiers).forEach(key => {
            const regex = new RegExp(`(@fires |[\{<\|,] ?!?|@typedef {.*} )${key}`, 'g');
            if (regex.test(comment.value)) {
              ...
              if (getModuleInfo(moduleId, parser)) {
                ...
                const delimiter = identifier.defaultImport ? '~' : getDelimiter(moduleId, exportName, parser);
                const replacement = `${moduleId.replace(slashRegEx, '/')}${exportName ? delimiter + exportName : ''}`;
